...
This document outlines End-to-End (E2E) test scenarios for the Visual Algorithm Designer frontend, covering the visual canvas, Algorithm Builder, and Circuit Designer.

## Visual Algorithm Designer Canvas

### Scenario: Drag A Learning Block To The Canvas

* **Title:** Student can place one learning block reliably.
* **Steps to Perform:**
    1. Navigate to the VAD canvas.
    2. Search for "path".
    3. Drag the Pathfinding learning block from the Explore library onto the central canvas.
    4. Release the pointer over the drop zone.
* **Expected Outcomes:**
    * Exactly one node is created.
    * The node lands near the release point.
    * The Lesson Inspector shows purpose, example, history, input/output shape, difficulty, and why it matters.

### Scenario: Connect Two Learning Blocks

* **Title:** Student can connect two nodes without duplicate or self edges.
* **Steps to Perform:**
    1. Place Search and Recommendation blocks on the canvas.
    2. Drag from the Search output handle to the Recommendation input handle.
    3. Try to connect the same handles again.
    4. Try to connect a node to itself.
* **Expected Outcomes:**
    * The first connection succeeds and gives visible edge feedback.
    * The duplicate connection is rejected.
    * The self-connection is rejected.

### Scenario: Starter Kit Expands As A Prefab

* **Title:** Student can drag a starter kit and receive all planned nodes and edges.
* **Steps to Perform:**
    1. Open Starter kits.
    2. Drag Game Map Route to the canvas.
* **Expected Outcomes:**
    * All expected nodes are created once.
    * The expected edges are created once.
    * The canvas remains usable after the drop state is cleared.
