...
const NODE_DROP_OFFSET = { x: 95, y: 48 };
  ...
  const onConnect = useCallback(
    (connection: Parameters<typeof addEdge>[0]) => {
      ...
      setEdges((current) => {
        ...
        return addEdge({ ...connection, animated: true }, current);
      });
    },
    [setEdges],
  );
  ...
  const addDragPayloadAt = useCallback(
    (position: { x: number; y: number }) => {
      if (dragPayload?.isPrefab) {
        ...
        addAlgorithmAt(dragPayload.algorithmId, {
          x: position.x - NODE_DROP_OFFSET.x,
          y: position.y - NODE_DROP_OFFSET.y,
        }, {
