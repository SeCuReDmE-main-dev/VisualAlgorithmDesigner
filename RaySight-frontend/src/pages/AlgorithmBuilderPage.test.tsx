...
describe('AlgorithmBuilderPage - Core Logic', () => {
  ...
  test('generateAlgorithm functionality: generates steps from text and clears input', async () => {
    ...
    const newStepInput = screen.getByLabelText(/New Step Description/i);
    const addButton = screen.getByRole('button', { name: /Add Step/i });
    fireEvent.change(newStepInput, { target: { value: 'Initial step to be replaced' } });
    await user.click(addButton);
    ...
    fireEvent.change(llmInput, { target: { value: 'Step A\nStep B\n\nStep C' } });
  ...
  test('deleteStep functionality: removes a step', async () => {
    ...
    fireEvent.change(newStepInput, { target: { value: 'Step to keep' } });
    await user.click(addButton);
    fireEvent.change(newStepInput, { target: { value: 'Step to delete' } });
    await user.click(addButton);
